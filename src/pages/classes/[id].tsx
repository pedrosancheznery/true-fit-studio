...
export const getServerSideProps: GetServerSideProps<ClassDetailsProps> = async (context) => {
  ...
  const supabaseServer = getSSRClient(context);
  ...
  const instancesWithCapacity =
    instancesRes.data?.map((instance) => ({
      ...instance,
      seatsRemaining: classCapacity - (bookingsByInstance.get(instance.id) ?? 0),
      isBooked: bookedInstanceIds.has(instance.id),
    })) ?? [];
              ...
              >
                <div className="flex flex-col items-start">
                  <span className="font-medium">
                    {new Date(instance.date).toLocaleDateString('en-GB', {
                      weekday: 'long',
                      day: 'numeric',
                    })}
                  </span>
